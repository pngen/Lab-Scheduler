...
// Fixture builders for the examples: same shapes as the test fixtures, kept
// separate so the examples do not depend on the test tree.
...
inline ResourceAdvertisement base(ResourceId id, ResourceClass resource_class, std::string name, WorkerId worker,
                                  WorkerBootId boot, const std::string& host) {
...
inline ResourceAdvertisement gpu(std::uint64_t id, std::uint64_t worker, std::uint64_t boot,
                                 const std::string& host, std::uint32_t slots) {
    ResourceAdvertisement advertisement =
        base(ResourceId::from_value(id), ResourceClass::Gpu, "gpu-" + decimal_u64(id),
    ...
    accelerator.model = "example-accelerator";
...
inline ResourceAdvertisement model(std::uint64_t id, std::uint64_t worker, std::uint64_t boot,
                                   const std::string& host, std::uint64_t model_id, const std::string& version,
                                   bool resident) {
    ResourceAdvertisement advertisement =
        base(ResourceId::from_value(id), ResourceClass::Model, "model-" + decimal_u64(id),
             WorkerId::from_value(worker), WorkerBootId::from_value(boot), host);
...
inline ResourceAdvertisement dataset(std::uint64_t id, std::uint64_t worker, std::uint64_t boot,
                                     const std::string& host, std::uint64_t dataset_id, std::uint64_t version) {
    ResourceAdvertisement advertisement =
        base(ResourceId::from_value(id), ResourceClass::Dataset, "dataset-" + decimal_u64(id),
             WorkerId::from_value(worker), WorkerBootId::from_value(boot), host);
    ...
    descriptor.digest = "sha256:example";
    advertisement.dataset = descriptor;
    advertisement.locality.local_dataset_versions.push_back(descriptor.version);
    return advertisement;
}

inline ResourceAdvertisement simulator(std::uint64_t id, std::uint64_t worker, std::uint64_t boot,
                                       const std::string& host, std::uint64_t simulator_id,
                                       const std::string& version) {
    ResourceAdvertisement advertisement =
        base(ResourceId::from_value(id), ResourceClass::Simulator, "sim-" + decimal_u64(id),
             WorkerId::from_value(worker), WorkerBootId::from_value(boot), host);
    advertisement.capacity.slots = 2;
    advertisement.capacity.max_sessions = 2;
...
inline ResourceAdvertisement virtual_environment(std::uint64_t id, std::uint64_t worker, std::uint64_t boot,
                                                 const std::string& host, std::uint64_t environment_id,
                                                 const std::string& digest) {
    ResourceAdvertisement advertisement =
        base(ResourceId::from_value(id), ResourceClass::VirtualEnvironment, "venv-" + decimal_u64(id),
             WorkerId::from_value(worker), WorkerBootId::from_value(boot), host);
    advertisement.capacity.slots = 4;
    advertisement.capacity.max_sessions = 4;
...
inline ResourceAdvertisement physical_rig(std::uint64_t id, std::uint64_t worker, std::uint64_t boot,
                                          const std::string& host, std::uint64_t environment_id) {
    ResourceAdvertisement advertisement =
        base(ResourceId::from_value(id), ResourceClass::PhysicalEnvironment, "rig-" + decimal_u64(id),
             WorkerId::from_value(worker), WorkerBootId::from_value(boot), host);
    advertisement.sharing = SharingMode::Exclusive;
    EnvironmentDescriptor descriptor;
    descriptor.id = EnvironmentId::from_value(environment_id);
    descriptor.physical_environment = PhysicalEnvironmentId::from_value(0x1501);
    descriptor.eligibility_token = "calibrated";
    advertisement.environment = descriptor;
    return advertisement;
}

inline ResourceRequirement requirement(ResourceClass resource_class, std::string name) {
...
inline ScheduleRequest request(std::uint64_t id, std::uint64_t experiment,
    ...
    request.workload = "example-workload";
    request.tenant = "example-tenant";
